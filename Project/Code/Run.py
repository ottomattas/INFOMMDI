...
daily_bias_dict = defaultdict(int)
total_biases_dict = {}
current_date = df['created'][0].strftime("%d/%m/%Y")

for idx, row in df.iterrows():
    if current_date != row['created'].strftime("%d/%m/%Y"):
        # total_biases.append((current_date, (np.sum(daily_bias) / n_comments_per_day)))
        total_biases_dict[current_date] = {k: v / n_comments_per_day for (k, v) in daily_bias_dict.items()}
        print("#" * 20 + str({idx}) + "#" * 20,
              f"\nDay {current_date} had these biases per word:\n "
              f"{[('Word: ' + w, 'Value: ' + str(f)) for (w, f) in daily_bias_dict.items()]}"
    ...
    for word in str(comment).split():
        if word in neg_bias_dict.keys():
            daily_bias_dict[word] += neg_bias_dict[word]
    n_comments_per_day += 1

print("Writing to file...")

pd.DataFrame.from_dict(total_biases_dict).fillna(0).T.to_csv("Datasets/daily_all.csv")

# %%
# Bin data
bins = np.linspace(np.min(df['created']), np.max(df['created']), 1000)
...
for bin_index, end_index in count_dict.items():
    ...
    bias_list_of_tuples.append((df['created'][end], bin_normed_bias))
    # bias_list_of_tuples.append((bin_index, bin_normed_bias))

    start = end

# create DataFrame using data
pd.DataFrame(bias_list_of_tuples, columns=['time', 'score']).to_csv("Datasets/for_tableau.csv")
